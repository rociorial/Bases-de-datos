...
-- 33. Haz una consulta que muestre la zona, número de parques, calle, número y metros de 
-- solar de las viviendas que se encuentran en una zona que posea más de un parque.
...
-- 34. Haz una consulta que muestre para cada vivienda unifamiliar la calle, número, plantas, 
-- metros del solar y metros construidos.
...
-- hayan construido bloques de pisos.
...
-- 36. Haz una consulta que muestre para cada piso la calle, número, planta, puerta, número de 
-- habitaciones, metros útiles, nombre de zona, número de parques existentes en la zona y nombre 
-- y apellidos del propietario.
...
-- 39. Haz una consulta que muestre los pisos (toda la información de la tabla piso) y el nombre 
-- completo de los propietarios que se encuentran en una zona con dos parques que tienen entre 2 
-- y 4 habitaciones o que se encuentran en la zona Centro, con ascensor y que tienen más de 70 y 
-- menos de 110 metros cuadrados útiles.
...
-- 40. Haz una consulta que muestre el nombre en minúsculas y las viviendas unifamiliares de una 
-- planta, que poseen los hombres de los cuales tenemos teléfono.
SELECT LOWER(NOMBRE), NUMERO, CALLE
FROM PROPIETARIO PP INNER JOIN CASAPARTICULAR CP ON PP.DNI = CP.DNIPROPIETARIO
WHERE TELEFONO IS NOT NULL

-- 41. Haz una consulta que muestre las viviendas (calle, numero y tipovivienda) y la zona urbana 
-- en la que se encuentran (nombrezona y descripción). 
