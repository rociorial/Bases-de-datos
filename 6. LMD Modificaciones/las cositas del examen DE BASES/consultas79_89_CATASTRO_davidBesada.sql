...
SELECT 
    PR.NOMBRE + ' ' + PR.APELLIDO1 + ' ' + PR.APELLIDO2 + ' posee un ' + 
    CASE 
        WHEN P.CALLE IS NOT NULL THEN 'piso' 
        ELSE 'casa' 
    END + 
    ' en la calle ' + 
    ISNULL(P.CALLE, V.CALLE) + ' nº ' + 
    ISNULL(CAST(P.NUMERO AS VARCHAR), CAST(V.NUMERO AS VARCHAR))
FROM 
    PROPIETARIO PR
LEFT JOIN 
    PISO P ON PR.DNI = P.DNIPROPIETARIO
LEFT JOIN 
    CASAPARTICULAR V ON PR.DNI = V.DNIPROPIETARIO;
